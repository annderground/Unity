...
using UnityEngine;
/// <summary>
///  this class moves the blue ball and detects collisions with the target or data points
/// </summary>
public class MoveBallBlue : MonoBehaviour
{
    //this is the rigidbody of the blue ball
    private Rigidbody _rb;

    // the manager collects the data of the whole game
    public Game manager;
    
    // in this list we save the data point collision times
    public List<string> timePoints;
    
    // we need those variables to control the movement of the ball
    public float forceMultiplier;
    public float jumpStrength;

    // we need the initial position of the ball to reset the game for the next round
    private Transform _startTransform;
    public static Vector3 InitialPosition;
    
    /// <summary>
    /// we save the initial ball position for later resetting the game
    ///
    /// we also check if there is a rigidbody of the blue ball
    /// </summary>
    private void Start()
    {
    ...
    /// <summary>
    /// ball can jump for no apparent reason
    /// </summary>
    private void Update()
    {
    ...
    public void OnTriggerEnter(Collider other)
    {
        ...
        if (other.CompareTag("targetBlue"))
        {
            manager.BlueArrived();
        } 
